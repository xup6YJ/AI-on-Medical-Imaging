...
class COVIDDataset(SubjectsDataset):
    def __init__(
        ...
        if not validation:
            data_path = '/media/yclin/3TBNAS/Medical-AI/LAB3/data/train'
        else:
            data_path = '/media/yclin/3TBNAS/Medical-AI/LAB3/data/valid'

        id_list = os.listdir(data_path)
        id_list = sorted(id_list)
        # print(id_list)
        image_paths = [glob.glob(data_path + '/CT/*.nii')][0]
        ...
        roi_paths = [glob.glob(data_path + '/Infection/*.nii')][0]
        icv_paths = [glob.glob(data_path + '/Lung/*.nii')][0]
        # roi_paths = [path.replace('CT', 'Infection') for path in image_paths]
        # icv_paths = [path.replace('CT', 'Lung') for path in image_paths]
        ...
        super(COVIDDataset, self).__init__(subjects=subjects, transform=transform, **kwargs)  ##
        
        
# dataset for 52 testing data
class COVIDTestDataset(SubjectsDataset):
    '''
    ...
    def __init__(self, **kwargs):
        ...
        transform = tio.transforms.Compose(transform)
        super(COVIDTestDataset, self).__init__(subjects=subjects, transform=transform, **kwargs)  ##
